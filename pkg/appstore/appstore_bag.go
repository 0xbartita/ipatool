...
func (t *appstore) Bag(input BagInput) (BagOutput, error) {
	...
	// (which the plist parser chokes on as "unexpected hex digit 'h'"), and
	// only the appstored UA gets a (download-grade) token. With the slash, the
	// Configurator UA mints a commerce-grade token that buyProduct accepts.
